# R Punycoder Package User Story

## Executive Summary

Development of `punycoder` - a high-performance R package for Unicode/Punycode encoding and decoding with a C++ backend using Rcpp, specifically designed for robust URL parsing and internationalized domain name (IDN) handling.
...
punycoder::url_encode("https://пример.испытание/path?q=test")
# → "https://xn--e1afmkfd.xn--80akhbyknj4f/path?q=test"

# Parse URLs with IDN handling
url_parts <- punycoder::parse_url("https://café.example.com/menu")
...
punycoder::puny_encode(domains)

# Data frame integration
urls_df$ascii_domains <- punycoder::puny_encode(urls_df$unicode_domains)
```
...
result <- punycoder::puny_encode("invalid..domain")
...
│   ├── punycoder.cpp  # Main Rcpp wrapper
│   ├── url_parser.cpp # URL parsing logic
│   ├── Makevars.in    # Template for optional libidn2 flags
│   └── Makevars(.win) # Build configuration + fallback defaults
├── man/              # Documentation (generated by roxygen2)
│   ├── punycoder-package.Rd
│   ├── puny_encode.Rd
│   └── ...
...
options(punycoder.strict = TRUE)      # Global option setting
getOption("punycoder.strict")         # Option retrieval
```
...
test_vectors <- read.csv(system.file("testdata", "rfc3492_vectors.csv", package = "punycoder"))
...
test_that("error handling", {
  ...
  expect_error(puny_decode("invalid-punycode"), class = "punycoder_error")
})
```
...
```makefile
PKG_CPPFLAGS = -I../inst/include @LIBIDN2_CFLAGS@
PKG_LIBS = @LIBIDN2_LIBS@

# configure script:
# - detects libidn2 via pkg-config
# - sets -DPUNYCODER_USE_LIBIDN2 when available
# - keeps fallback backend when unavailable
```
...
- [x] Integrate optional libidn2 backend with fallback C++ implementation
- [x] Create basic Rcpp wrappers
- [x] Implement core encode/decode functions
- [x] Add comprehensive error handling

### Phase 2: API Development (Week 3)
- [x] URL-aware functions
- [x] Input validation system
- [x] Vectorized operations
- [x] Documentation with roxygen2

### Phase 3: Testing & Polish (Week 4)
- [x] RFC 3492 test vector validation
- [x] Performance optimization
- [x] CRAN compliance check
- [x] Vignette creation

### Phase 4: Release Preparation (Week 5)
- [x] Final testing on multiple platforms
- [x] Code review and cleanup
- [ ] CRAN submission preparation
- [x] GitHub repository setup

R-hub progress update (February 21, 2026):
- [x] https://github.com/bart-turczynski/punycoder/actions/runs/22258182777 (green)
- [x] https://github.com/bart-turczynski/punycoder/actions/runs/22258395920 (green)

## Success Metrics

### Technical Success
- [x] 100% RFC 3492 test vector compliance
- [x] >10,000 domains/second processing speed
...
- **Optional external dependency** (`libidn2`) with built-in fallback backend
