...
range_start <- which(grepl(", First>$", name_vec))
...
canon_decomp <- list()
has_decomp <- which(nzchar(decomp_f) & !grepl("<", decomp_f))
for (i in has_decomp) {
...
idna_lo <- integer(0); idna_hi <- integer(0); idna_status <- integer(0)
idna_map <- list()
for (line in idna) {
  segs <- trimws(strsplit(line, ";", fixed = TRUE)[[1]])
  rng <- strsplit(segs[[1]], "\\.\\.")[[1]]
  ...
  idna_lo <- c(idna_lo, lo); idna_hi <- c(idna_hi, hi)
  idna_status <- c(idna_status, status_code[[st]])
  idna_map[[length(idna_map) + 1L]] <- mapping
}
ord <- order(idna_lo)
idna_lo <- idna_lo[ord]; idna_hi <- idna_hi[ord]
idna_status <- idna_status[ord]; idna_map <- idna_map[ord]
...
to_ranges <- function(cps, values) {
  o <- order(cps); cps <- cps[o]; values <- values[o]
  ...
  while (i <= n) {
    ...
    lo <- c(lo, cps[i]); hi <- c(hi, cps[j]); val <- c(val, values[i])
    i <- j + 1L
  }
...
chunk <- function(strs, per = 8L) {
  if (!length(strs)) return("")
  idx <- (seq_along(strs) - 1L) %/% per
  paste(vapply(split(strs, idx), function(g) paste0("  ", paste(g, collapse = ", ")),
               character(1)), collapse = ",\n")
}

guard <- "PUNYCODER_UNICODE_TABLES_16_0_0_H"
header <- sprintf('// Generated by data-raw/generate_unicode_tables.R. DO NOT EDIT BY HAND.
...
flat <- integer(0); offs <- integer(0); lens <- integer(0)
for (s in decomp_seqs) {
  offs <- c(offs, length(flat)); lens <- c(lens, length(s)); flat <- c(flat, s)
}

emit_ccc <- paste(sprintf("{%s, %s, %d}", hexlit(ccc_ranges$lo), hexlit(ccc_ranges$hi),
                          ccc_ranges$value), collapse = ", ")
src <- c(
  ...
  sprintf('const CccRange CCC_RANGES[] = {\n%s\n};',
          chunk(sprintf("{%s, %s, %d}", hexlit(ccc_ranges$lo),
                        hexlit(ccc_ranges$hi), ccc_ranges$value), 4L)),
  sprintf('const size_t CCC_N = %d;', nrow(ccc_ranges)),
  '',
  '// --- Combining-mark ranges (Mn/Mc/Me, sorted by lo) ---',
  'struct MarkRange { uint32_t lo, hi; };',
  sprintf('const MarkRange MARK_RANGES[] = {\n%s\n};',
          chunk(sprintf("{%s, %s}", hexlit(mark_ranges$lo), hexlit(mark_ranges$hi)), 6L)),
  ...
  sprintf('const DecompEntry DECOMP_INDEX[] = {\n%s\n};',
  ...
  sprintf('const CompEntry COMP_TABLE[] = {\n%s\n};',
          chunk(sprintf("{%s, %s, %s}", hexlit(comp_a), hexlit(comp_b), hexlit(comp_c)), 3L)),
...
idna_flat <- integer(0); idna_off <- integer(0); idna_len <- integer(0)
for (m in idna_map) {
  idna_off <- c(idna_off, length(idna_flat)); idna_len <- c(idna_len, length(m))
  idna_flat <- c(idna_flat, m)
}
src <- c(src,
  sprintf('const IdnaRange IDNA_RANGES[] = {\n%s\n};',
          chunk(sprintf("{%s, %s, %d, %d, %d}", hexlit(idna_lo), hexlit(idna_hi),
                        idna_status, idna_off, idna_len), 3L)),
