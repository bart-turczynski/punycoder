...
| URL surface *(deprecated)* | [`url_encode()`](https://bart-turczynski.github.io/punycoder/reference/url_encode.md), [`url_decode()`](https://bart-turczynski.github.io/punycoder/reference/url_decode.md), [`parse_url()`](https://bart-turczynski.github.io/punycoder/reference/parse_url.md) | Best-effort host rewriting. Slated for removal — see ADR-006. |
...
| `url-utils.R` | Deprecated URL surface; `.deprecate_url_surface()` emits the [`.Deprecated()`](https://rdrr.io/r/base/Deprecated.html) warning. |
...
representative URLs; those tests `skip_if` libidn2 is unavailable.
...
- **New host/IDNA behavior** → `host_normalize` / `puny_*`, never the
  deprecated URL surface (ADR-006). URL parsing belongs upstack in
  `rurl`.
...
- **Backend-specific code** → `punycoder_backend.cpp` only; never
  sprinkle `#ifdef PUNYCODER_USE_LIBIDN2` through domain/URL code
  (ADR-008).
