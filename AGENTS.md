...
These tables were 56% of `host_normalize`'s process time before PUNY-zgaqusnu. The seven accessors now share one `range_lookup` binary search (plus `key_lookup` for the point-keyed decomposition index; composition is keyed on a *pair* and keeps its own), and each answers ASCII without searching — the two tables that cover ASCII (UTS #46 mapping, `Bidi_Class`) via a 128-entry direct index, the rest via a bounds test against their own first/last listed code point. `canonical_compose` bounds its **second** element only, because `a` can be ASCII (smallest is U+003C) while `b` is always a combining character.

**Every one of those constants and arrays is derived in the generator** from the same UCD vectors the search reads, so the fast path cannot drift from the slow one and a version bump moves the boundaries automatically. Do not hand-write a boundary into the emitted C++, and do not "simplify" a derived constant to a literal — the generator also asserts the shape it assumed and will stop rather than emit a guard that skips real data (ADR-011). The gate for any change here is an element-wise `host_normalize` diff over `unique(c(source, to_ascii))` from `inst/testdata/IdnaTestV2.txt` across all 8 flag combinations.
