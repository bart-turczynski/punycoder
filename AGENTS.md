...
Which table set the public surface uses is `kDefaultUnicodeVersion` (currently 16.0.0); `normalization_profile_info()` reports it. Moving it is a reviewed behavior change (`dev/normalization-contract.md` §8), separate from adding a table set. `punycoder:::.host_normalize_version()` and `punycoder:::.unicode_versions()` are internal test hooks for reaching a non-default set — not API.
