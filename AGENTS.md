...
`punycoder` is an R package providing RFC 3492-compliant Punycode/IDN encode/decode functions plus a UTS #46 canonical-host normalizer, implemented in C++ via Rcpp. The Punycode codec optionally links against `libidn2` when present at build time; otherwise it uses an in-tree fallback algorithm. Normalization (NFC + UTS #46) is always in-tree, built on vendored Unicode tables pinned to one Unicode version (currently 17.0.0). It is the Punycode/IDNA engine for the `pslr` and `rurl` packages.
...
Normalization depends on vendored Unicode tables (`src/unicode_tables_<tag>.{h,cpp}`) generated by `data-raw/generate_unicode_tables.R`. **Network access happens only at generation time** — the generated C++ is committed and the package never downloads anything at build or run time. Every output name (filenames, header guard, namespace) derives from the single version string passed to that script; downloaded UCD files are cached per version under the git-ignored `data-raw/.ucd-cache/<version>/`.

The version is a **required argument** — `Rscript data-raw/generate_unicode_tables.R 17.0.0` — and the script holds no default, so there is no resting value to go stale (PUNY-ktgyxlqd). It emits one version per run, unlike `data-raw/fetch_idna_fixtures.R`, which loops over the shipped set; the generator can't loop because it emits C++ from multi-line string templates whose leading whitespace is part of the output, so wrapping its body would re-indent them. Regenerating everything is one invocation per version.

**More than one table set ships at once (ADR-015): 16.0.0 and 17.0.0 are both compiled in.** To add another, run the generator with the new version, re-run it for each already-shipped version and confirm `git diff src/` is empty for those files (the generator is reproducible — that check is what proves the new set didn't perturb the old ones) — then add the `#include` to `src/unicode_tables_registry.h` and the `X(...)` row to `PUNYCODER_UNICODE_VERSIONS` in `src/punycoder_unicode_version.h`. Do generation and registration in **one commit**: an unreferenced table object still links into the `.so` (R builds pass no `--gc-sections`), so splitting them costs ~250 KB for nothing. Doing only one of the two edits fails loudly — a missing `X(...)` means the version does not exist, a missing `#include` is a compile error where the facade column expands, and a row without an instantiation is a `-Wswitch` warning plus a link error.
